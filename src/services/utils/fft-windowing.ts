// This file is taken and modified from https://github.com/eleven-i/fft-windowing-ts/blob/master/
  ...
  kaiser: (n: number, points: number, alpha: number = 3) => {
    return (
      bessi0(
        Math.PI *
          alpha *
          Math.sqrt(1 - Math.pow((2 * n) / (points - 1) - 1, 2)),
...
const applyWindowFunction = (
  ...
  for (let n = 0; n < datapoints; ++n) {
    ...
    data_array[n] *= windowingFunction(n, datapoints, alpha);
  }
 ...
 */
const createWindowFunction =
  (win: WindowFunctionName) => (array: Float32Array, alpha?: number) =>
    applyWindowFunction(array, windows[win], alpha);
/**
 * Adds a function for each window to the module exports.
 */
const hann = createWindowFunction("hann");
const hamming = createWindowFunction("hamming");
const cosine = createWindowFunction("cosine");
const lanczos = createWindowFunction("lanczos");
const gaussian = createWindowFunction("gaussian");
const tukey = createWindowFunction("tukey");
const blackman = createWindowFunction("blackman");
const exact_blackman = createWindowFunction("exact_blackman");
const kaiser = createWindowFunction("kaiser");
const nuttall = createWindowFunction("nuttall");
const blackman_harris = createWindowFunction("blackman_harris");
const blackman_nuttall = createWindowFunction("blackman_nuttall");
const flat_top = createWindowFunction("flat_top");
...
/**
 * Applies a window function to an array. Does not take alpha as a parameter.
 */
export const window = (win: WindowFunctionName, array: Float32Array) =>
  createWindowFunction(win)(array);
